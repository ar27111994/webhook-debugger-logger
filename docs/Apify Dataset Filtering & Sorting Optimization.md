...
Performing filtering *inside* the Node.js application after fetching data from Apify is an "anti-pattern" for large datasets.
...
*Note: This architecture introduces CPU overhead. For high-throughput scenarios, isolates should be reused, or a "snapshot" of the compiled context should be used.*.9
...
**Recommendation:** If the primary use case is *filtering and sorting* large flat datasets, **DuckDB** provides the highest performance-to-cost ratio. If the data is highly nested or requires frequent single-item updates, **MongoDB** is preferable.
...
* **Pre-Filtering:** SQL filters eliminate data *before* it hits the expensive JS sandbox.  
...
* **Pagination Design:** Prefer **Cursor-based pagination** (e.g., since\_id) over offset pagination for the user-facing API. It is stable (records don't shift if new data is added) and efficient (![][image5]).21
...
13. Tuning Workloads \- DuckDB, accessed on January 24, 2026, [https://duckdb.org/docs/stable/guides/performance/how\_to\_tune\_workloads](https://duckdb.org/docs/stable/guides/performance/how_to_tune_workloads)  
