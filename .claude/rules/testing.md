...
- Enforce strict staged coverage gates with `npm run coverage:gate`
- Ensure matrix scope remains at 100% for statements, branches, functions, and lines
- Test edge cases, error paths, and retries

## Known Runtime Signals (Not Failures by Default)

- `initialize() called again without a preceding shutdown()` in lifecycle-heavy tests
- `Shutting down` with `TEST_COMPLETE` during harness teardown
- Background timeout logs (for resilience/timeout-path validation)
- JSON parse warnings in sanitation/fallback tests
- `Force exiting Jest...` in coverage flows using `--forceExit`

Treat these as actionable only when accompanied by assertion failures, unhandled exceptions, or non-zero coverage gate failures.
...
npm run test:coverage:new-scopes      # Scoped coverage run
npm run coverage:check:new-scopes     # Scoped threshold validation
npm run test:coverage:matrix          # Full matrix coverage run
npm run coverage:check:matrix         # Full matrix threshold validation
npm run coverage:gate                 # Full staged gate
```
