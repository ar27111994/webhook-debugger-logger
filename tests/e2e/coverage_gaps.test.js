...
await setupCommonMocks({ apify: true, ssrf: true, logger: true });
...
describe("Coverage Improvement Tests", () => {
  ...
  test("should log error when SSRF blocks validation during forwarding", async () => {
    ...
    // Wait for async processing to log the error via structured logger
    ...
    expect(validateUrlForSsrf).toHaveBeenCalledWith(forwardUrl);
  ...
  test("should handle SSE initial write failure", async () => {
    ...
    try {
      ...
      expect(loggerMock.error).toHaveBeenCalledWith(
        expect.objectContaining({
          err: expect.objectContaining({ message: "Simulated Write Error" }),
        }),
  ...
  test("should log 500 errors in global error handler", async () => {
    ...
    expect(loggerMock.error).toHaveBeenCalledWith(
      expect.objectContaining({
        status: 500,
        err: expect.objectContaining({ message: "Simulated Server Crash" }),
      }),
      "Server error",
    );
      ...
      expect(loggerMock.error).toHaveBeenCalledWith(
        expect.objectContaining({
          error: expect.objectContaining({
            message: "Simulated Broadcast Error",
          }),
        }),
        "Failed to broadcast message to client",
      );
