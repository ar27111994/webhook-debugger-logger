...
const { apifyMock, logRepositoryMock, loggerMock, constsMock } = await import('../setup/helpers/shared-mocks.js');
const keyValueStoreMock = await apifyMock.openKeyValueStore();
const { WebhookManager } = await import('../../src/webhook_manager.js');
const { APP_CONSTS, WEBHOOK_ID_PREFIX, ENV_VARS, ENV_VALUES } = await import('../../src/consts/app.js');
...
describe('WebhookManager', () => {
    ...
    describe('generateWebhooks', () => {
        it('should generate requested number of webhooks', async () => {
            ...
            expect(keyValueStoreMock.setValue).toHaveBeenCalledWith(KVS_KEYS.STATE, expect.any(Object));
        });

        it('should fail if count is negative or invalid', async () => {
            await expect(manager.generateWebhooks(INVALID_COUNT_NEG, TEST_RETENTION)).rejects.toThrow(ERROR_MESSAGES.INVALID_COUNT(INVALID_COUNT_NEG));
            await expect(manager.generateWebhooks(INVALID_COUNT_FLOAT, TEST_RETENTION)).rejects.toThrow(ERROR_MESSAGES.INVALID_COUNT(INVALID_COUNT_FLOAT));
        });

        it('should fail if count exceeds max bulk create', async () => {
            const tooMany = APP_CONSTS.MAX_BULK_CREATE + 1;
            await expect(manager.generateWebhooks(tooMany, TEST_RETENTION)).rejects.toThrow(ERROR_MESSAGES.INVALID_COUNT_MAX(tooMany, APP_CONSTS.MAX_BULK_CREATE));
        });

        it('should fail if retention is invalid', async () => {
            await expect(manager.generateWebhooks(1, INVALID_RETENTION_NEG)).rejects.toThrow(ERROR_MESSAGES.INVALID_RETENTION(INVALID_RETENTION_NEG));
            // @ts-expect-error - Testing invalid input
            await expect(manager.generateWebhooks(1, String(TEST_RETENTION))).rejects.toThrow(ERROR_MESSAGES.INVALID_RETENTION(String(TEST_RETENTION)));
        });
    });
    ...
    describe('isValid / getAllActive', () => {
        it('should accurately report validity based on expiration', async () => {
            ...
            manager.addWebhookForTest('wh_active', { expiresAt: new Date(Date.now() + ONE_HOUR_MS).toISOString() });
            manager.addWebhookForTest('wh_expired', { expiresAt: new Date(Date.now() - ONE_HOUR_MS).toISOString() });
        ...
        it('should list all active webhooks accurately', async () => {
            ...
            manager.addWebhookForTest('wh_active', { expiresAt: new Date(Date.now() + ONE_HOUR_MS).toISOString() });
            manager.addWebhookForTest('wh_active2', { expiresAt: new Date(Date.now() + ONE_HOUR_MS).toISOString() });
            manager.addWebhookForTest('wh_expired', { expiresAt: new Date(Date.now() - ONE_HOUR_MS).toISOString() });
    ...
    describe('cleanup', () => {
        ...
        it('should purge expired webhooks and their resources', async () => {
            ...
            expect(logRepositoryMock.findOffloadedPayloads).toHaveBeenCalledWith('wh_expired');

            // Should have deleted KVS records
            expect(keyValueStoreMock.setValue).toHaveBeenCalledWith('offload_1', null);
            expect(keyValueStoreMock.setValue).toHaveBeenCalledWith('offload_2', null);

            // Should have deleted logs by Webhook ID
            expect(logRepositoryMock.deleteLogsByWebhookId).toHaveBeenCalledWith('wh_expired');
        ...
        it('should safely swallow and log DB deletion failures', async () => {
            logRepositoryMock.deleteLogsByWebhookId.mockRejectedValue(new Error('DB Failed'));
    ...
    describe('updateRetention', () => {
        ...
        beforeEach(() => {
            ...
            manager.addWebhookForTest('wh_active', { expiresAt: new Date(Date.now() + ONE_HOUR_MS).toISOString() }); // 1 hr future
            manager.addWebhookForTest('wh_expired', { expiresAt: new Date(Date.now() - ONE_HOUR_MS).toISOString() }); // 1 hr past
        });
        ...
        it('should extend expiration of active webhooks only', async () => {
            ...
            const activeData = manager.getWebhookData('wh_active') || { expiresAt: '' };
            const expiredData = manager.getWebhookData('wh_expired') || { expiresAt: '' };

            expect(new Date(activeData.expiresAt).getTime()).toBe(expectedFutureMs);
            expect(new Date(expiredData.expiresAt).getTime()).toBeLessThan(Date.now()); // Remains unmodified
        ...
        it('should disregard shortening retention to prevent accidental loss', async () => {
            ...
            expect(new Date(activeData.expiresAt).getTime()).toBe(Date.now() + ONE_HOUR_MS);
        ...
        it('should reject invalid retentions explicitly', async () => {
            await expect(manager.updateRetention(INVALID_RETENTION_NEG)).rejects.toThrow(ERROR_MESSAGES.INVALID_RETENTION(INVALID_RETENTION_NEG));
            await expect(manager.updateRetention(INVALID_ZERO)).rejects.toThrow(ERROR_MESSAGES.INVALID_RETENTION(INVALID_ZERO));
            // @ts-expect-error - Testing invalid input
            await expect(manager.updateRetention('hello')).rejects.toThrow(ERROR_MESSAGES.INVALID_RETENTION('hello'));
        });
    });
    ...
    describe('Test Mode Utility Functions', () => {
        ...
        it('adds webhooks artificially only in test mode', () => {
            process.env[ENV_VARS.NODE_ENV] = ENV_VALUES.TEST;
            manager.addWebhookForTest('test_1', { expiresAt: new Date().toISOString() });
            expect(manager.hasWebhook('test_1')).toBe(true);

            process.env[ENV_VARS.NODE_ENV] = ENV_VALUES.PRODUCTION;
            manager.addWebhookForTest('test_2', { expiresAt: new Date().toISOString() });
            expect(manager.hasWebhook('test_2')).toBe(false);
        });

        it('checks existence only in test mode', () => {
            process.env[ENV_VARS.NODE_ENV] = ENV_VALUES.TEST;
            manager.addWebhookForTest('test_3', { expiresAt: new Date().toISOString() });
