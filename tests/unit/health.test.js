...
describe('Health Routes', () => {
    ...
    describe('/ready (Readiness Probe)', () => {
        ...
        it('should return 200 OK when DB is connected and Webhooks exist', async () => {
            jest.mocked(getDbInstance).mockResolvedValueOnce(assertType({ connected: true }));
            const mockGetActiveWebhookCount = jest.fn().mockReturnValue(MOCK_WEBHOOK_COUNT);

            const { ready } = createHealthRoutes(assertType(mockGetActiveWebhookCount));
            const mockReq = createMockRequest();
            const mockRes = createMockResponse();
            ...
            expect(mockRes.json).toHaveBeenCalledWith(
                expect.objectContaining({
                    ...
                    checks: {
                        ...
                        webhooks: { status: STATUS_LABELS.OK, message: `${MOCK_WEBHOOK_COUNT} active webhook(s)` }
                    }
                })
            );
        });
