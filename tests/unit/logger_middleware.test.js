...
import { HTTP_HEADERS, HTTP_STATUS, HTTP_METHODS, MIME_TYPES, RECURSION_HEADER_NAME, RECURSION_HEADER_VALUE } from '../../src/consts/http.js';
...
const { LoggerMiddleware, createLoggerMiddleware } = await import('../../src/logger_middleware.js');
...
function createMockExpress() {
    ...
    const res = createMockResponse(assertType({
...
describe('LoggerMiddleware', () => {
    ...
    beforeEach(() => {
        ...
        jest.mocked(webhookManagerMock.getWebhookData).mockReturnValue(assertType({}));
        jest.spyOn(webhookRateLimiter, 'check').mockReturnValue({ allowed: true, remaining: 99, resetMs: 1000 });
    ...
    describe('Instantiation & Options', () => {
        it('should correctly create and assign options', () => {
            ...
            const middleware = new LoggerMiddleware(webhookManagerMock, rawOptions, onEventMock, forwardingServiceMock);
        ...
        it('should compile scripts dynamically if provided', () => {
            const middleware = new LoggerMiddleware(webhookManagerMock, { customScript: `event.statusCode = ${HTTP_STATUS.CREATED};` }, onEventMock, forwardingServiceMock);
            expect(middleware.hasCompiledScript()).toBe(true);
        });

        it('factory pattern exports correctly configured middleware object', () => {
            const middlewareFn = createLoggerMiddleware(webhookManagerMock, {}, onEventMock);
    ...
    describe('ingestMiddleware', () => {
        ...
        beforeEach(() => {
            middlewareInstance = new LoggerMiddleware(webhookManagerMock, {}, onEventMock, forwardingServiceMock);
        });
        ...
        it('should block recursive loop requests', async () => {
            ...
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ message: ERROR_MESSAGES.RECURSIVE_FORWARDING }));
            expect(next).not.toHaveBeenCalled();
        });

        it('should block requests over max size limits dynamically', async () => {
            const mw = new LoggerMiddleware(webhookManagerMock, { maxPayloadSize: 100 }, onEventMock, forwardingServiceMock);
            const { req, res, next } = createMockExpress();
        ...
        it('should handle rate limiting gracefully', async () => {
            const resetMs = 5000;
            const { req, res, next } = createMockExpress();
            jest.mocked(webhookRateLimiter.check).mockReturnValue({ allowed: false, remaining: 0, resetMs });

            await middlewareInstance.ingestMiddleware(req, res, next);

            expect(res.status).toHaveBeenCalledWith(HTTP_STATUS.TOO_MANY_REQUESTS);
            expect(res.setHeader).toHaveBeenCalledWith(HTTP_HEADERS.RETRY_AFTER, resetMs / APP_CONSTS.MS_PER_SECOND);
            expect(next).not.toHaveBeenCalled();
        });
    });
    ...
    describe('middleware execution (main logic)', () => {
        ...
        let mw;
        beforeEach(() => {
            mw = new LoggerMiddleware(webhookManagerMock, {
                defaultResponseCode: HTTP_STATUS.OK,
                defaultResponseBody: { ok: true }
            }, onEventMock, forwardingServiceMock);
        });
        ...
        it('should reject invalid webhooks with 404', async () => {
            ...
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ error: ERROR_MESSAGES.WEBHOOK_NOT_FOUND }));
        });
        ...
        it('should execute basic flow and emit events without error', async () => {
            ...
            expect(appEvents.emit).toHaveBeenCalledWith(EVENT_NAMES.LOG_RECEIVED, expect.any(Object));
            expect(onEventMock).toHaveBeenCalled();
        });

        it('should validate schemas returning 400 if mismatch', async () => {
            const strictMw = new LoggerMiddleware(webhookManagerMock, {
        ...
        it('should run custom transformation scripts cleanly', async () => {
            ...
            const scriptMw = new LoggerMiddleware(webhookManagerMock, {
        ...
        it('should trigger webhook forwarding natively', async () => {
            ...
            const fwdMw = new LoggerMiddleware(webhookManagerMock, {
                forwardUrl: fwdUrl
            }, onEventMock, forwardingServiceMock);
