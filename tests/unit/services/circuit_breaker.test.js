...
describe("CircuitBreaker", () => {
  ...
  describe("initialization", () => {
    ...
    it("should perform periodic cleanup", async () => {
      ...
      jest.advanceTimersByTime(
        FORWARDING_CONSTS.CIRCUIT_BREAKER_CLEANUP_INTERVAL_MS + bufferMs,
      );
  ...
  describe("state management", () => {
    ...
    it("should open circuit when threshold exceeded", () => {
      ...
      for (
        let i = 0;
        i < FORWARDING_CONSTS.CIRCUIT_BREAKER_FAILURE_THRESHOLD;
        i++
      ) {
        circuitBreaker.recordFailure(TARGET_URL);
      }
    ...
    it("should do nothing if recordSuccess is called for a hostname not in states", () => {
      expect(() =>
        circuitBreaker.recordSuccess("https://not-in-state.com"),
      ).not.toThrow();
    });
  ...
  describe("prune", () => {
    it("should remove expired entries with no failures", () => {
      ...
      circuitBreaker.states.set(TARGET_HOSTNAME, {
        failures: 0,
        nextAttempt:
          Date.now() - FORWARDING_CONSTS.CIRCUIT_BREAKER_CLEANUP_INTERVAL_MS, // In the past
      });
    ...
    it("should NOT remove active failed states", () => {
      circuitBreaker.states.set(TARGET_HOSTNAME, {
        failures: 1,
        nextAttempt:
          Date.now() + FORWARDING_CONSTS.CIRCUIT_BREAKER_RESET_TIMEOUT_MS, // In the future
      });
    ...
    it("should enforce maxSize limit via LRU-ish eviction", () => {
      ...
      const futureMs = 1000;
      circuitBreaker.states.set("a.com", {
        failures: 1,
        nextAttempt: Date.now() + futureMs,
      });
      circuitBreaker.states.set("b.com", {
        failures: 1,
        nextAttempt: Date.now() + futureMs,
      });
      circuitBreaker.states.set("c.com", {
        failures: 1,
        nextAttempt: Date.now() + futureMs,
      });
  ...
  describe("Half-Open Recovery and Time Drift", () => {
    it("should allow a request after reset timeout, but re-open on immediate failure", () => {
      ...
      const bufferMs = 10;
      jest.advanceTimersByTime(
        FORWARDING_CONSTS.CIRCUIT_BREAKER_RESET_TIMEOUT_MS + bufferMs,
      );
  ...
  describe("Security and Sanitation Checks", () => {
    ...
    it("should handle non-string types safely through the catch block", () => {
      expect(() =>
        circuitBreaker.recordFailure(assertType(null)),
      ).not.toThrow();
      expect(() =>
        circuitBreaker.recordFailure(assertType(undefined)),
      ).not.toThrow();
    ...
    it("should normalize hostnames irrespective of casing or port", () => {
      ...
      circuitBreaker.recordFailure(
        `HTTPS://${TARGET_HOSTNAME.toUpperCase()}:8080/foo`,
      );
