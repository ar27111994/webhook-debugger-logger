...
import { useMockCleanup, useFakeTimers } from "../../setup/helpers/test-lifecycle.js";
import { assertType, createMockRequest, flushPromises } from "../../setup/helpers/test-utils.js";
...
const { FORWARDING_CONSTS, APP_CONSTS } = await import("../../../src/consts/app.js");
const { HTTP_STATUS, HTTP_METHODS, HTTP_HEADERS, RECURSION_HEADER_NAME, RECURSION_HEADER_VALUE } = await import("../../../src/consts/http.js");
const { ERROR_MESSAGES, ERROR_LABELS } = await import("../../../src/consts/errors.js");
const { LOG_MESSAGES } = await import("../../../src/consts/messages.js");
const { axiosMock, apifyMock, ssrfMock, loggerMock } = await import("../../setup/helpers/shared-mocks.js");
const { ForwardingService } = await import("../../../src/services/ForwardingService.js");

const DEFAULT_MAX_RETRIES = 3;
const SHORT_MAX_RETRIES = 2;
const CIRCUIT_BREAKER_FAILURE_COUNT = FORWARDING_CONSTS.CIRCUIT_BREAKER_FAILURE_THRESHOLD;
...
describe("ForwardingService", () => {
    ...
    beforeEach(() => {
        ...
        // Default safe SSRF
        ssrfMock.validateUrlForSsrf.mockResolvedValue({ safe: true, href: TEST_URL_HTTP, host: TEST_URL_HOST });
    ...
    describe("sendSafeRequest()", () => {
        const defaultHeaders = { [HTTP_HEADERS.CONTENT_TYPE]: MIME_TYPES.JSON, "x-secret": "hidden" };
        const defaultUrl = TEST_URL_HTTPS;

        it("should send a basic request and return response on success", async () => {
            const mockResponse = { status: HTTP_STATUS.OK, data: HTTP_STATUS_MESSAGES[HTTP_STATUS.OK] };
            mockAxiosInstance.request.mockResolvedValue(mockResponse);

            const result = await service.sendSafeRequest(defaultUrl, HTTP_METHODS.POST, { ok: 1 }, defaultHeaders, {});
        ...
        it("should strip FORWARD_HEADERS_TO_IGNORE when forwardHeaders is true", async () => {
            ...
            const headers = { "host": "old.com", [HTTP_HEADERS.CONTENT_LENGTH]: "100", "safe-header": "test" };
            await service.sendSafeRequest(defaultUrl, HTTP_METHODS.POST, {}, headers, { forwardHeaders: true });
        ...
        it("should only forward Content-Type when forwardHeaders is false", async () => {
            mockAxiosInstance.request.mockResolvedValue({ status: HTTP_STATUS.OK });
            const headers = { [HTTP_HEADERS.CONTENT_TYPE]: MIME_TYPES.JSON, "safe-header": "test" };
            await service.sendSafeRequest(defaultUrl, HTTP_METHODS.POST, {}, headers, { forwardHeaders: false });

            const reqConfig = mockAxiosInstance.request.mock.calls[0][0];
            expect(reqConfig.headers).toHaveProperty(HTTP_HEADERS.CONTENT_TYPE, MIME_TYPES.JSON);
        ...
        it("should inject a custom hostHeader override if provided", async () => {
            ...
            mockAxiosInstance.request.mockResolvedValue({ status: HTTP_STATUS.OK });
            await service.sendSafeRequest(defaultUrl, HTTP_METHODS.POST, {}, {}, { hostHeader: newHost });
        ...
        it("should immediately throw ERROR_MESSAGES.ABORTED if signal is pre-aborted", async () => {
            ...
            await expect(
                service.sendSafeRequest(defaultUrl, HTTP_METHODS.POST, {}, {}, {}, abortController.signal)
        ...
        it("should throw immediately on axios cancellation and NOT retry", async () => {
            ...
            Object.defineProperty(cancelError, "isCancel", { value: true, configurable: true });
        ...
        it("should classify 500/502/503/504 as transient HTTP errors and retry", async () => {
            ...
            mockAxiosInstance.request.mockResolvedValue({ status: HTTP_STATUS.SERVICE_UNAVAILABLE });

            const requestPromise = service.sendSafeRequest(defaultUrl, HTTP_METHODS.POST, {}, {}, { maxRetries: DEFAULT_MAX_RETRIES });

            // It sleeps for backoff between retries
            await flushPromises();
            jest.advanceTimersByTime(FORWARDING_CONSTS.RETRY_BASE_DELAY_MS);
            await flushPromises();
            jest.advanceTimersByTime(FORWARDING_CONSTS.RETRY_BASE_DELAY_MS * FORWARDING_CONSTS.RETRY_BACKOFF_BASE);
            await flushPromises();
            ...
            // 1 initial + 2 retries
            expect(mockAxiosInstance.request).toHaveBeenCalledTimes(DEFAULT_MAX_RETRIES);
        });
        ...
        it("should classify ECONNRESET/ETIMEDOUT as transient network errors and retry", async () => {
            ...
            const requestPromise = service.sendSafeRequest(defaultUrl, HTTP_METHODS.POST, {}, {}, { maxRetries: SHORT_MAX_RETRIES });
            ...
            expect(mockAxiosInstance.request).toHaveBeenCalledTimes(SHORT_MAX_RETRIES);
        });

        it("should NOT retry 4xx errors (non-transient) and throw immediately, tripping CB", async () => {
            mockAxiosInstance.request.mockResolvedValue({ status: HTTP_STATUS.NOT_FOUND });
            ...
            await expect(
                service.sendSafeRequest(defaultUrl, HTTP_METHODS.POST, {}, {}, { maxRetries: DEFAULT_MAX_RETRIES })
        ...
        it("should NOT follow redirects and instead throw as a failure (Security)", async () => {
            ...
            mockAxiosInstance.request.mockResolvedValue({ status: HTTP_STATUS.MOVED_PERMANENTLY });
        ...
        it("should exhaust maxRetries and throw the last encountered error", async () => {
            ...
            const promise = service.sendSafeRequest(defaultUrl, HTTP_METHODS.POST, {}, {}, { maxRetries: SHORT_MAX_RETRIES });
        ...
        it("should record a successful request against the circuit breaker", async () => {
            mockAxiosInstance.request.mockResolvedValue({ status: HTTP_STATUS.OK, data: HTTP_STATUS_MESSAGES[HTTP_STATUS.OK] });
        ...
        it("should wire up the abort signal to clear the retry delay timeout", async () => {
            ...
            const requestPromise = service.sendSafeRequest(defaultUrl, HTTP_METHODS.POST, {}, {}, { maxRetries: DEFAULT_MAX_RETRIES }, abortController.signal);
        ...
        it("should throw REPLAY_ATTEMPTS_EXHAUSTED fallback if loop finishes without throwing", async () => {
            ...
            const maxRetries = -1;
            await expect(
                service.sendSafeRequest(defaultUrl, HTTP_METHODS.POST, {}, {}, { maxRetries })
        ...
        it("should log FORWARD_TIMEOUT if axiosError code is TIMEOUT_CODE", async () => {
            ...
            await expect(
                service.sendSafeRequest(defaultUrl, HTTP_METHODS.POST, {}, {}, { maxRetries: 1 })
        ...
        it("should handle environment without timer.unref safely", async () => {
            ...
            const p = service.sendSafeRequest(defaultUrl, HTTP_METHODS.POST, {}, {}, { maxRetries: 2 });
            await flushPromises();
    ...
    describe("forwardWebhook()", () => {
        ...
        it("should auto-prefix standard HTTP to URLs missing a protocol", async () => {
            ...
            expect(ssrfMock.validateUrlForSsrf).toHaveBeenCalledWith(`http://${forwardUrl}`);
            expect(mockAxiosInstance.request).toHaveBeenCalledWith(
        ...
        it("should immediately abort if Circuit Breaker is open for the URL", async () => {
            ...
            for (let i = 0; i < CIRCUIT_BREAKER_FAILURE_COUNT; i++) service.circuitBreaker.recordFailure(TEST_BAD_URL);

            await service.forwardWebhook(mockEvent, mockReq, mockOptions, TEST_BAD_URL);
        ...
        it("should abort if the calculated body length exceeds MAX_ALLOWED_PAYLOAD_SIZE", async () => {
            mockReq.headers[HTTP_HEADERS.CONTENT_LENGTH] = String(APP_CONSTS.MAX_ALLOWED_PAYLOAD_SIZE + MOCK_CONTENT_LENGTH);
            await service.forwardWebhook(mockEvent, mockReq, mockOptions, TEST_URL_HTTP);
        ...
        it("should push error data to Actor when sending fails", async () => {
            ...
            await service.forwardWebhook(mockEvent, mockReq, mockOptions, TEST_URL_HTTPS);
        ...
        it("should classify 500 errors as transient and include transient label in pushData", async () => {
            ...
            expect(apifyMock.pushData).toHaveBeenCalledWith(
                expect.objectContaining({
                    ...
                    webhookId: mockEvent.webhookId,
                    type: ERROR_LABELS.FORWARD_ERROR,
                    body: expect.not.stringContaining("Non-transient error/Permanent Failure")
                })
            );
            expect(apifyMock.pushData).toHaveBeenCalledWith(
                expect.objectContaining({
                    ...
                    webhookId: mockEvent.webhookId,
                    type: ERROR_LABELS.FORWARD_ERROR,
                    body: expect.stringContaining(`Request failed with status code ${HTTP_STATUS.INTERNAL_SERVER_ERROR}`)
                })
            );
        });
        ...
        it("should swallow Actor.pushData failures silently rather than crashing the background worker", async () => {
            ...
            const promise = service.forwardWebhook(mockEvent, mockReq, mockOptions, TEST_URL_HTTPS);
        ...
        it("should gracefully ignore if string length calculation triggers", async () => {
            ...
            delete mockReq.headers[HTTP_HEADERS.CONTENT_LENGTH];
            mockAxiosInstance.request.mockResolvedValue({ status: HTTP_STATUS.OK });
            await service.forwardWebhook(mockEvent, mockReq, mockOptions, TEST_URL_HOST);
            expect(mockAxiosInstance.request).toHaveBeenCalled();
        });
        ...
        it("should fallback to default maxRetries if options.maxForwardRetries is undefined", async () => {
            ...
            mockAxiosInstance.request.mockResolvedValue({ status: HTTP_STATUS.OK });
            await service.forwardWebhook(mockEvent, mockReq, optionsNoRetries, TEST_URL_HTTPS);
            expect(mockAxiosInstance.request).toHaveBeenCalled();
        });

        it("should fallback to validatedUrl if ssrfResult.href is missing", async () => {
            ssrfMock.validateUrlForSsrf.mockResolvedValue({ safe: true, host: TEST_URL_HOST }); // no href
            mockAxiosInstance.request.mockResolvedValue({ status: HTTP_STATUS.OK });
            await service.forwardWebhook(mockEvent, mockReq, mockOptions, TEST_URL_HTTPS);
            expect(mockAxiosInstance.request).toHaveBeenCalledWith(
        ...
        it("should fallback to empty string if ssrfResult.host is missing", async () => {
            ssrfMock.validateUrlForSsrf.mockResolvedValue({ safe: true, href: TEST_URL_HTTPS }); // no host
            mockAxiosInstance.request.mockResolvedValue({ status: HTTP_STATUS.OK });
            await service.forwardWebhook(mockEvent, mockReq, mockOptions, TEST_URL_HTTPS);
            expect(mockAxiosInstance.request).toHaveBeenCalledWith(
        ...
        it("should return immediately and not pushError if signal is aborted during catch block", async () => {
            ...
            await service.forwardWebhook(mockEvent, mockReq, mockOptions, TEST_URL_HTTPS, controller.signal);
        ...
        it("should correctly trip the circuit breaker under concurrent failing requests (Stress/Concurrency)", async () => {
            ...
            ssrfMock.validateUrlForSsrf.mockResolvedValue({ safe: true, href: failingUrl, host: "unstable-target.com" });
        ...
        describe("hardening (audit feedback)", () => {
            it("should clear Prior circuit breaker failures on success (recordSuccess)", async () => {
                ...
                expect(service.circuitBreaker.states?.get?.(host)?.failures).toBe(CIRCUIT_BREAKER_FAILURE_COUNT - 1);
            ...
            it("should pass HTTPS URLs to validateUrlForSsrf unchanged", async () => {
                ...
                expect(ssrfMock.validateUrlForSsrf).toHaveBeenCalledWith(TEST_URL_HTTPS);
            });
            ...
            it("should NOT trip circuit breaker on first transient failure but trip on exhaustion", async () => {
                ...
                // Run with retries
                const maxRetries = 2;
                const promise = service.sendSafeRequest(url, HTTP_METHODS.POST, {}, {}, { maxRetries });
            ...
            it("should rethrow and skip retries if signal aborted mid-request (catch block)", async () => {
                ...
                const promise = service.sendSafeRequest(url, HTTP_METHODS.POST, {}, {}, { maxRetries: 3 }, controller.signal);
            ...
            it("should default forwardHeaders to true when options.forwardHeaders is undefined", async () => {
                ...
                await service.sendSafeRequest(TEST_URL_HTTPS, HTTP_METHODS.POST, {}, headers, options);
