...
const { verifySignature, createStreamVerifier, finalizeStreamVerification } = await import('../../src/utils/signature.js');
...
describe('Signature Utils (Real Crypto)', () => {
    ...
    describe('verifySignature', () => {
        it('should return error if secret is missing', () => {
            const result = verifySignature({ provider: SIGNATURE_PROVIDERS.CUSTOM }, PAYLOAD, {});
        ...
        it('should fail if calculated signature does not match expected', () => {
            ...
            const result = verifySignature({ provider: SIGNATURE_PROVIDERS.STRIPE, secret: SECRET }, PAYLOAD, headers);
            expect(result.valid).toBe(false);
        ...
        describe('Stripe', () => {
            ...
            it('should fail missing Stripe header', () => {
                const result = verifySignature({ provider: SIGNATURE_PROVIDERS.STRIPE, secret: SECRET }, PAYLOAD, {});
                expect(result.valid).toBe(false);
            ...
            it('should catch exceptions in getProviderContext', () => {
                ...
                const result = verifySignature({ provider: SIGNATURE_PROVIDERS.STRIPE, secret: SECRET }, PAYLOAD, assertType(undefined));
                expect(result.valid).toBe(false);
                expect(result.error).toContain('TypeError');
            });

            it('should fail malformed Stripe header', () => {
                const headers = { [HTTP_HEADERS.STRIPE_SIGNATURE]: `t=${Math.floor(Date.now() / APP_CONSTS.MS_PER_SECOND)}` }; // Missing v1
                const result = verifySignature({ provider: SIGNATURE_PROVIDERS.STRIPE, secret: SECRET }, PAYLOAD, headers);
                expect(result.valid).toBe(false);
            ...
            it('should fail if timestamp is too old', () => {
                ...
                const oldTimestamp = Math.floor(Date.now() / APP_CONSTS.MS_PER_SECOND) - tenMinutesInSeconds;
                const header = createStripeSignature(oldTimestamp, PAYLOAD, SECRET);
                const headers = { [HTTP_HEADERS.STRIPE_SIGNATURE]: header };
                const result = verifySignature({ provider: SIGNATURE_PROVIDERS.STRIPE, secret: SECRET, tolerance: SIGNATURE_CONSTS.TOLERANCE_SECONDS }, PAYLOAD, headers);
                expect(result.valid).toBe(false);
        ...
        describe('Shopify', () => {
            it('should verify Shopify signatures', () => {
                ...
                const result = verifySignature({ provider: SIGNATURE_PROVIDERS.SHOPIFY, secret: SECRET }, PAYLOAD, headers);
                expect(result.valid).toBe(true);
            });

            it('should fail missing Shopify header', () => {
                const result = verifySignature({ provider: SIGNATURE_PROVIDERS.SHOPIFY, secret: SECRET }, PAYLOAD, {});
                expect(result.valid).toBe(false);
                expect(result.error).toContain(SIGNATURE_ERRORS.MISSING_HEADER);
            });
            ...
            it('should fail Shopify if timestamp is too old', () => {
                const tenMinutesInSeconds = 600;
                const oldTimestamp = new Date(Date.now() - tenMinutesInSeconds * APP_CONSTS.MS_PER_SECOND).toISOString();
                const header = createShopifySignature(PAYLOAD, SECRET);
                const headers = { [HTTP_HEADERS.SHOPIFY_HMAC_SHA256]: header, [HTTP_HEADERS.SHOPIFY_TRIGGERED_AT]: oldTimestamp };
        ...
        describe('GitHub', () => {
            it('should verify GitHub signatures', () => {
                ...
                const result = verifySignature({ provider: SIGNATURE_PROVIDERS.GITHUB, secret: SECRET }, PAYLOAD, headers);
                expect(result.valid).toBe(true);
            });

            it('should fail missing GitHub header', () => {
                const result = verifySignature({ provider: SIGNATURE_PROVIDERS.GITHUB, secret: SECRET }, PAYLOAD, {});
                expect(result.valid).toBe(false);
                expect(result.error).toContain(SIGNATURE_ERRORS.MISSING_HEADER);
            });
            ...
            it('should fail invalid GitHub prefix', () => {
                ...
                const signature = crypto.createHmac(HASH_ALGORITHMS.SHA256, SECRET).update(PAYLOAD).digest(assertType(SIGNATURE_ENCODINGS.HEX));
                const headers = { [HTTP_HEADERS.HUB_SIGNATURE_256]: `${SIGNATURE_PREFIXES.SHA1}${signature}` };
                const result = verifySignature({ provider: SIGNATURE_PROVIDERS.GITHUB, secret: SECRET }, PAYLOAD, headers);
                expect(result.valid).toBe(false);
                expect(result.error).toContain(SIGNATURE_ERRORS.INVALID_FORMAT);
            });
        ...
        describe('Slack', () => {
            it('should verify Slack signatures', () => {
                ...
                const result = verifySignature({ provider: SIGNATURE_PROVIDERS.SLACK, secret: SECRET }, PAYLOAD, headers);
                expect(result.valid).toBe(true);
            });
            ...
            it('should fail invalid Slack prefix', () => {
                ...
                // eslint-disable-next-line sonarjs/hardcoded-secret-signatures
                const signature = crypto.createHmac(HASH_ALGORITHMS.SHA256, SECRET).update(`${SIGNATURE_PREFIXES.V0_NO_PREFIX}:${timestamp}:${PAYLOAD}`).digest(assertType(SIGNATURE_ENCODINGS.HEX));
                const headers = {
        ...
        describe('Custom Provider', () => {
            it('should verify Custom signatures', () => {
                // eslint-disable-next-line sonarjs/hardcoded-secret-signatures
                const signature = crypto.createHmac(HASH_ALGORITHMS.SHA256, SECRET).update(PAYLOAD, ENCODINGS.UTF).digest(assertType(SIGNATURE_ENCODINGS.HEX));
                const headers = { [customHeaderName.toLowerCase()]: signature };
                const result = verifySignature(
                    { provider: SIGNATURE_PROVIDERS.CUSTOM, secret: SECRET, headerName: customHeaderName },
                    PAYLOAD,
                    headers
                );
                expect(result.valid).toBe(true);
            ...
            it('should fail Custom if headerName configuration is missing', () => {
                const result = verifySignature({ provider: SIGNATURE_PROVIDERS.CUSTOM, secret: SECRET }, PAYLOAD, {});
                expect(result.valid).toBe(false);
            ...
            it('should fail Custom if signature header is missing in request', () => {
                const result = verifySignature({ provider: SIGNATURE_PROVIDERS.CUSTOM, secret: SECRET, headerName: customHeaderName }, PAYLOAD, {});
                expect(result.valid).toBe(false);
                expect(result.error).toBe(SIGNATURE_ERRORS.MISSING_CUSTOM_HEADER(customHeaderName));
            });
            ...
            it('should support timestamp validation for custom providers (ISO 8601)', () => {
                ...
                // eslint-disable-next-line sonarjs/hardcoded-secret-signatures
                const signature = crypto.createHmac(HASH_ALGORITHMS.SHA256, SECRET).update(PAYLOAD, ENCODINGS.UTF).digest(assertType(SIGNATURE_ENCODINGS.HEX));
                const headers = { [customHeaderName.toLowerCase()]: signature, [customTimestampKey.toLowerCase()]: validTimestamp };
                const result = verifySignature(
                    { provider: SIGNATURE_PROVIDERS.CUSTOM, secret: SECRET, headerName: customHeaderName, timestampKey: customTimestampKey },
                    PAYLOAD,
                    headers
                );
                expect(result.valid).toBe(true);
            ...
            it('should hit provider fallback when timestamp is too old without provider', () => {
                ...
                // eslint-disable-next-line sonarjs/hardcoded-secret-signatures
                const signature = crypto.createHmac(HASH_ALGORITHMS.SHA256, SECRET).update(PAYLOAD, ENCODINGS.UTF).digest(assertType(SIGNATURE_ENCODINGS.HEX));
                const headers = { [customHeaderName.toLowerCase()]: signature, [customTimestampKey.toLowerCase()]: oldTimestamp };
                const result = verifySignature(
                    { secret: SECRET, headerName: customHeaderName, timestampKey: customTimestampKey },
                    PAYLOAD,
                    headers
                );
                expect(result.valid).toBe(false);
            ...
            it('should handle completely unparseable timestamps', () => {
                // eslint-disable-next-line sonarjs/hardcoded-secret-signatures
                const signature = crypto.createHmac(HASH_ALGORITHMS.SHA256, SECRET).update(PAYLOAD, ENCODINGS.UTF).digest(assertType(SIGNATURE_ENCODINGS.HEX));
                const headers = { [customHeaderName.toLowerCase()]: signature, [customTimestampKey.toLowerCase()]: 'not-a-date' };
                const result = verifySignature(
                    { provider: SIGNATURE_PROVIDERS.CUSTOM, secret: SECRET, headerName: customHeaderName, timestampKey: customTimestampKey },
                    PAYLOAD,
                    headers
        ...
        describe('Generic Error Handling', () => {
            it('should handle unknown providers', () => {
                const result = verifySignature({ provider: 'UNKNOWN_MAGIC', secret: SECRET }, PAYLOAD, {});
                expect(result.valid).toBe(false);
    ...
    describe('createStreamVerifier', () => {
        it('should return initial hmac for streaming', () => {
            const header = createGitHubSignature(PAYLOAD, SECRET);
            const headers = { [HTTP_HEADERS.HUB_SIGNATURE_256]: header };
            const verifier = createStreamVerifier({ provider: SIGNATURE_PROVIDERS.GITHUB, secret: SECRET }, headers);

            expect(verifier.hmac).toBeDefined();
            // expectedSignature should be extracted without the sha256= prefix
            const expectedSig = header.replace(assertType(SIGNATURE_PREFIXES.SHA256), '');
        ...
        it('should return error if secrets are missing', () => {
            const verifier = createStreamVerifier({ provider: SIGNATURE_PROVIDERS.CUSTOM }, {});
        ...
        it('should return error if context has error', () => {
            // Missing headers
            const verifier = createStreamVerifier({ provider: SIGNATURE_PROVIDERS.GITHUB, secret: SECRET }, {});
            expect(verifier.hmac).toBeNull();
        ...
        it('should apply prefix for streaming with Stripe', () => {
            ...
            const header = createStripeSignature(timestamp, PAYLOAD, SECRET);
            const headers = { [HTTP_HEADERS.STRIPE_SIGNATURE]: header };
            const verifier = createStreamVerifier({ provider: SIGNATURE_PROVIDERS.STRIPE, secret: SECRET }, headers);
        ...
        it('should fallback to CUSTOM provider if omitted in createStreamVerifier', () => {
            ...
            const verifier = createStreamVerifier({ secret: SECRET, headerName: customHeaderName }, headers);
            expect(verifier.hmac).toBeDefined();
        });
        ...
        it('should pre-validate timestamp during stream creation', () => {
            ...
            const header = createStripeSignature(oldTimestamp, PAYLOAD, SECRET);
            const headers = { [HTTP_HEADERS.STRIPE_SIGNATURE]: header };
            const verifier = createStreamVerifier({ provider: SIGNATURE_PROVIDERS.STRIPE, secret: SECRET, tolerance: SIGNATURE_CONSTS.TOLERANCE_SECONDS }, headers);
            expect(verifier.hmac).toBeNull();
        ...
        it('should hit provider fallback when timestamp is too old in createStreamVerifier', () => {
            ...
            // eslint-disable-next-line sonarjs/hardcoded-secret-signatures
            const signature = crypto.createHmac(HASH_ALGORITHMS.SHA256, SECRET).update(PAYLOAD, ENCODINGS.UTF).digest(assertType(SIGNATURE_ENCODINGS.HEX));
            const headers = { [customHeaderName.toLowerCase()]: signature, [customTimestampKey.toLowerCase()]: oldTimestamp.toString() };
            // Omit provider
            const verifier = createStreamVerifier({ secret: SECRET, headerName: customHeaderName, timestampKey: customTimestampKey }, headers);
            expect(verifier.hmac).toBeNull();
            expect(verifier.error).toContain(SIGNATURE_ERRORS.TIMESTAMP_TOLERANCE);
        });
        ...
        it('should catch unexpected errors during HMAC creation', () => {
            ...
            const spy = jest.spyOn(crypto, 'createHmac').mockImplementationOnce(() => {
                throw new Error(cryptoError);
            });

            const headers = { [HTTP_HEADERS.SHOPIFY_HMAC_SHA256]: 'dummy' };
            const verifier = createStreamVerifier({ provider: SIGNATURE_PROVIDERS.SHOPIFY, secret: SECRET }, headers);
    ...
    describe('Advanced Security & Concurrency', () => {
        it('should handle concurrency safely without sharing state', async () => {
            ...
            for (let i = 0; i < iterations; i++) {
                ...
                const header = createStripeSignature(timestamp, PAYLOAD, SECRET);
                const headers = { [HTTP_HEADERS.STRIPE_SIGNATURE]: header };
                tasks.push(Promise.resolve(verifySignature({ provider: SIGNATURE_PROVIDERS.STRIPE, secret: SECRET }, PAYLOAD, headers)));
            }
