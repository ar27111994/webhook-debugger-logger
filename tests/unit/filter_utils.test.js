...
describe('Filter Utils', () => {
    ...
    describe('parseRangeQuery', () => {
        it('should parse valid range operators', () => {
            expect(parseRangeQuery({ gt: '10' })).toEqual([{ operator: 'gt', value: TEST_VAL_10 }]);
            expect(parseRangeQuery({ lte: '5.5' })).toEqual([{ operator: 'lte', value: TEST_VAL_5_5 }]);
        });

        it('should handle simple scalar values (equality shorthand)', () => {
            expect(parseRangeQuery('10')).toEqual([{ operator: 'eq', value: TEST_VAL_10 }]);
            expect(parseRangeQuery(TEST_VAL_10)).toEqual([{ operator: 'eq', value: TEST_VAL_10 }]);
        });
        ...
        it('should handle type="string" explicitly', () => {
            expect(parseRangeQuery('val', 'string')).toEqual([{ operator: 'eq', value: 'val' }]);
            expect(parseRangeQuery({ ne: 123 }, 'string')).toEqual([{ operator: 'ne', value: '123' }]);
        });
    });

    describe('matchesRange', () => {
        it('should match correctly based on operator', () => {
            expect(matchesRange(TEST_VAL_10, [{ operator: 'gt', value: TEST_VAL_5 }])).toBe(true);
            expect(matchesRange(TEST_VAL_10, [{ operator: 'lt', value: TEST_VAL_5 }])).toBe(false);
            expect(matchesRange(TEST_VAL_10, [{ operator: 'eq', value: TEST_VAL_10 }])).toBe(true);

            // Branch coverage for other operators
            expect(matchesRange(TEST_VAL_10, [{ operator: 'gte', value: TEST_VAL_10 }])).toBe(true);
            expect(matchesRange(TEST_VAL_10, [{ operator: 'gte', value: 11 }])).toBe(false);
            expect(matchesRange(TEST_VAL_10, [{ operator: 'lte', value: TEST_VAL_10 }])).toBe(true);
            expect(matchesRange(TEST_VAL_10, [{ operator: 'lte', value: 5 }])).toBe(false);
            expect(matchesRange(TEST_VAL_10, [{ operator: 'ne', value: 5 }])).toBe(true);
            expect(matchesRange(TEST_VAL_10, [{ operator: 'ne', value: 10 }])).toBe(false);

            // Unknown operator
            expect(matchesRange(TEST_VAL_10, [{ operator: 'unknown', value: 10 }])).toBe(true);
        });

        it('should return false for invalid value types', () => {
            expect(matchesRange(assertType(null), [{ operator: 'eq', value: 1 }])).toBe(false); // value check
    ...
    describe('matchObject', () => {
        ...
        it('should return true for matching filters', () => {
            expect(matchObject(item, assertType({ status: HTTP_STATUS.OK }))).toBe(true);
        ...
        it('should handle non-string filter values (strict equality)', () => {
            expect(matchObject(item, assertType({ status: HTTP_STATUS.OK }))).toBe(true);
            expect(matchObject(item, assertType({ status: HTTP_STATUS.CREATED }))).toBe(false);
        });
        ...
        it('should handle null/undefined target object', () => {
            ...
            expect(matchObject(null, assertType({ status: HTTP_STATUS.OK }))).toBe(false);
            expect(matchObject(undefined, assertType({ status: HTTP_STATUS.OK }))).toBe(false);
            expect(matchObject(0, assertType({ status: HTTP_STATUS.OK }))).toBe(false);
            expect(matchObject(false, assertType({ status: HTTP_STATUS.OK }))).toBe(false);
            expect(matchObject('', assertType({ status: HTTP_STATUS.OK }))).toBe(false);
        });

        it('should return false for non-matching filters', () => {
            expect(matchObject(item, assertType({ status: HTTP_STATUS.NOT_FOUND }))).toBe(false);
        });
        ...
        it('should safely handle prototype pollution attempts in filter keys', () => {
            ...
            expect(typeof matchObject(protoObject, { toString: 'function' })).toBe('boolean');

            // Verify resilience against prototype pollution attempts
            expect(matchObject(item, assertType({ '__proto__.polluted': true }))).toBe(false);
        });
