/**
 * @file tests/unit/routes/replay.test.js
...
describe("Replay Routes", () => {
  ...
  beforeEach(() => {
    ...
    // Default happy paths
    mockSsrf.validateUrlForSsrf.mockResolvedValue({
      safe: true,
      href: MOCK_TARGET_URL,
      host: MOCK_TARGET_HOST,
    });
    mockLogRepo.getLogById.mockResolvedValue(
      assertType({
        id: MOCK_LOG_ID,
    ...
    mockForwardingService.sendSafeRequest.mockResolvedValue(
      assertType({
  ...
  describe("createReplayHandler", () => {
    ...
    it("should fallback natively if injected property accessors return undefined", async () => {
      ...
      const explicitlyUndefinedHandler = createReplayHandler(
        () => undefined,
        () => undefined,
      );
    ...
    it("should send INTERNAL_SERVER_ERROR if internal promise crashes naturally outside validation", async () => {
      ...
      expect(mockRes.json).toHaveBeenCalledWith(
        expect.objectContaining({ error: ERROR_LABELS.REPLAY_FAILED }),
      );
    });

    it("should return 400 if URL fails SSRF validation generally", async () => {
      const errorMessage = "SSRF_BLOCKED";
      mockSsrf.validateUrlForSsrf.mockResolvedValueOnce({
        safe: false,
        error: errorMessage,
      });
    ...
    it("should fallback to timestamp matching but handle empty items array safely", async () => {
      mockReq.params = {
        webhookId: "wh-123",
        itemId: "2023-01-01T00:00:00.000Z",
      }; // valid timestamp
      mockReq.query.url = MOCK_TARGET_URL;
      ...
      expect(mockLogRepo.findLogs).toHaveBeenCalledWith({
        timestamp: [
          { operator: SQL_CONSTS.OPERATORS.EQ, value: mockReq.params.itemId },
        ],
    ...
    it("should fallback to findLogs by timestamp if itemId maps to a Date value", async () => {
      ...
      mockLogRepo.findLogs.mockResolvedValue({
        items: assertType([timestampItem]),
        total: 1,
      });
      ...
      expect(mockRes.json).toHaveBeenCalledWith(
        expect.objectContaining({ targetResponseCode: 200 }),
      );
    });
    ...
    it("should log an error when KVS hydration crashes instead of skipping gracefully", async () => {
      ...
      mockLogRepo.getLogById.mockResolvedValueOnce(
        assertType({
          id: MOCK_LOG_ID,
          method: HTTP_METHODS.POST,
      ...
      mockApifyActor.getValue.mockRejectedValueOnce(
        new Error("KVS Error Simulation"),
      );
    ...
    it("should return a valid replay block if Axios responds via error response code natively", async () => {
      ...
      Object.assign(responseError, {
        response: { status: HTTP_STATUS.BAD_REQUEST, data: errorData },
      });
    ...
    it("should trigger AbortController signal when total replay timeout occurs", async () => {
      ...
      mockForwardingService.sendSafeRequest.mockImplementation(
        async (_url, _method, _data, _headers, _config, abortSignal) => {
          expect(abortSignal?.aborted).toBe(false);
          // Advance timers enough to trigger the totalTimeoutMs (10000ms * 11 = 110000)
          const MAX_REPLAY_ATTEMPTS_EXCEEDED = 15;
          const TIMEOUT_ADVANCE_MS =
            FORWARDING_CONSTS.FORWARD_TIMEOUT_MS * MAX_REPLAY_ATTEMPTS_EXCEEDED;
