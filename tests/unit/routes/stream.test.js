...
describe("Stream Route", () => {
  ...
  it("should establish an SSE connection successfully", () => {
    ...
    expect(mockRes.setHeader).toHaveBeenCalledWith(
      ...
      SECURITY_HEADERS_VALUES.NO_CACHE,
    );
    expect(mockRes.setHeader).toHaveBeenCalledWith(
      HTTP_HEADERS.CONNECTION,
      SECURITY_HEADERS_VALUES.KEEP_ALIVE,
    );
    expect(mockRes.setHeader).toHaveBeenCalledWith(
      HTTP_HEADERS.X_ACCEL_BUFFERING,
      SECURITY_HEADERS_VALUES.NO,
    );
    ...
    expect(mockReq.on).toHaveBeenCalledWith(
      STREAM_EVENTS.CLOSE,
      expect.any(Function),
    );
  });
  ...
  it("should cleanup client if the request is closed", () => {
    ...
    const closeHandler = jest
      .mocked(mockReq.on)
      .mock.calls.find((c) => c[0] === STREAM_EVENTS.CLOSE)?.[1];
