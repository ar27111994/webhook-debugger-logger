...
const { fsPromisesMock, loggerMock } =
  await import("../../setup/helpers/shared-mocks.js");
const { ensureLocalInputExists, getDefaultsFromSchema } =
  await import("../../../src/utils/bootstrap.js");
...
describe("bootstrap utils", () => {
  ...
  describe("getDefaultsFromSchema()", () => {
    ...
    it("should use APIFY_ACTOR_DIR env var to resolve schema path", async () => {
      ...
      expect(
        String(fsPromisesMock.readFile.mock.calls[0][0]).replaceAll("\\", "/"),
      ).toContain("/custom/dir");
    });
