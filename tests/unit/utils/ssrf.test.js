/**
 * @file tests/unit/utils/ssrf.test.js
...
describe("SSRF Utilities", () => {
  ...
  describe("validateUrlForSsrf", () => {
    ...
    it("should reject URLs resolving to blocked IPs", async () => {
      ...
      dnsPromisesMock.resolve4.mockResolvedValue(
        // eslint-disable-next-line sonarjs/no-hardcoded-ip
        assertType(["169.254.169.254"]),
      );
      dnsPromisesMock.resolve6.mockResolvedValue(assertType([]));
    ...
    it("should handle DNS resolution failure", async () => {
      dnsPromisesMock.resolve4.mockRejectedValue(
        assertType(new Error("ENOTFOUND")),
      );
    ...
    it("should handle direct IP literals correctly", async () => {
    ...
    it("should handle DNS timeout", async () => {
      ...
      dnsPromisesMock.resolve6.mockImplementation(
        assertType(() => new Promise(() => { })),
      );
    ...
    it("should handle invalid resolved IPs", async () => {
  ...
  describe("Advanced Security & Concurrency", () => {
    it("should handle concurrency safely", async () => {
      ...
      const results = await Promise.all(
        urls.map((url) => validateUrlForSsrf(url)),
      );
    ...
    describe("hardening (audit feedback)", () => {
      it("should correctly identify IPv6 loopback literal [::1] as internal", async () => {
        // eslint-disable-next-line sonarjs/no-clear-text-protocols
        const result = await validateUrlForSsrf("http://[::1]");
        expect(result.safe).toBe(false);
        expect(result.error).toBe(SSRF_ERRORS.INTERNAL_IP);
      });

      it("should correctly identify IPv6 mapped IPv4 loopback literal [::ffff:127.0.0.1] as internal", async () => {
        // eslint-disable-next-line sonarjs/no-clear-text-protocols
        const result = await validateUrlForSsrf("http://[::ffff:127.0.0.1]");
        expect(result.safe).toBe(false);
        expect(result.error).toBe(SSRF_ERRORS.INTERNAL_IP);
      });
    });
