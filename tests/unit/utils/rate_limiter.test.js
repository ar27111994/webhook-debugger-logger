/**
 * @file tests/unit/utils/rate_limiter.test.js
...
describe("RateLimiter", () => {
  ...
  describe("Constructor & Config", () => {
    it("should throw on invalid windowMs", () => {
      ...
      const invalidWindowMs = [0, negativeWindowMs, String(WINDOW_MS)];

      invalidWindowMs.forEach((windowMs) => {
        expect(() => new RateLimiter(LIMIT, assertType(windowMs))).toThrow(
          ERROR_MESSAGES.RATE_LIMITER_INVALID_WINDOW,
        );
      });
    });
    ...
    it("should throw on invalid maxEntries", () => {
      ...
      invalidMaxEntries.forEach((maxEntries) => {
        expect(
          () => new RateLimiter(LIMIT, WINDOW_MS, assertType(maxEntries)),
    ...
    it("should throw on invalid limit", () => {
      ...
      const invalidLimit = [negativeLimit, floatLimit];

      invalidLimit.forEach((limit) => {
        expect(() => new RateLimiter(assertType(limit), WINDOW_MS)).toThrow(
  ...
  describe("Helper Methods", () => {
    ...
    it("maskIp should handle obfuscation securely", () => {
      // eslint-disable-next-line sonarjs/no-hardcoded-ip
      expect(limiter.maskIp("192.168.1.50")).toBe(
        `192.168.1${LOG_MESSAGES.MASK_IPV4_SUFFIX}`,
      );
      // eslint-disable-next-line sonarjs/no-hardcoded-ip
      expect(limiter.maskIp("2001:0db8:85a3:0000:0000:8a2e:0370:7334")).toBe(
        `2001:0db8${LOG_MESSAGES.MASK_IPV6_SUFFIX}`,
      );
  ...
  describe("Middleware rate limiting", () => {
    ...
    it("should block requests consistently exceeding limit", () => {
      ...
      expect(res.setHeader).toHaveBeenCalledWith(
        ...
        Math.ceil(WINDOW_MS / APP_CONSTS.MS_PER_SECOND),
      );
    });
    ...
    it("should return 400 when missing IP identifier", () => {
      req = createMockRequest({
        ip: undefined,
        headers: { [HTTP_HEADERS.X_SIMULATE_NO_IP]: "true" },
      });
      mw(req, res, next);
    ...
    it("should log eviction when maxEntries limits are met in non-test env", () => {
      ...
      // Track second IP, should evict first and log
      strictMw(
        // eslint-disable-next-line sonarjs/no-hardcoded-ip
        createMockRequest({ ip: "2.2.2.2" }),
        createMockResponse(),
        createMockNextFunction(),
      );
      ...
      expect(loggerMock.info).toHaveBeenCalledWith(
        expect.objectContaining({
          evictedIp: `1.1.1${LOG_MESSAGES.MASK_IPV4_SUFFIX}`,
          maxEntries: 1,
        }),
  ...
  describe("Cleanup Job & Edge cases", () => {
    ...
    it("should prune entirely stale IPs over time", () => {
      ...
      jest.advanceTimersByTime(
        windowMs + timeToAdvanceSec * APP_CONSTS.MS_PER_SECOND,
      );
    ...
    it("should keep fresh timestamps but drop stale ones inside the array", () => {
      ...
      // IP is still there, but array is smaller (hits line: this.#hits.set(key, fresh))
    ...
    it("should log prunedCount > 0 in non-test environments", () => {
      ...
      const mw = clockLimiter.middleware();
      mw(
        createMockRequest({ ip: "drop.me.entirely" }),
        createMockResponse(),
        createMockNextFunction(),
      );
  ...
  describe("hardening (audit feedback)", () => {
    it("should fallback to req.socket.remoteAddress when req.ip is missing", () => {
      // eslint-disable-next-line sonarjs/no-hardcoded-ip
      const socketIp = "5.5.5.5";
      const reqNoIp = createMockRequest({
        ip: undefined,
        socket: assertType({ remoteAddress: socketIp })
      });
      const resLocal = createMockResponse();
      const nextLocal = createMockNextFunction();

      limiter.middleware()(reqNoIp, resLocal, nextLocal);

      expect(nextLocal).toHaveBeenCalled();
      expect(limiter.hasIp(socketIp)).toBe(true);
    });

    it("should prioritize X-Forwarded-For over X-Real-IP when trustProxy is true", () => {
      const proxyLimiter = new RateLimiter(LIMIT, WINDOW_MS, MAX_ENTRIES, true);
      // eslint-disable-next-line sonarjs/no-hardcoded-ip
      const forwardedIp = "1.1.1.1";
      // eslint-disable-next-line sonarjs/no-hardcoded-ip
      const realIp = "2.2.2.2";
      const reqBoth = createMockRequest({
        headers: {
          [HTTP_HEADERS.X_FORWARDED_FOR]: forwardedIp,
          [HTTP_HEADERS.X_REAL_IP]: realIp
        }
      });
      const resLocal = createMockResponse();
      const nextLocal = createMockNextFunction();

      proxyLimiter.middleware()(reqBoth, resLocal, nextLocal);

      expect(proxyLimiter.hasIp(forwardedIp)).toBe(true);
      expect(proxyLimiter.hasIp(realIp)).toBe(false);
      proxyLimiter.destroy();
    });

    it("should use X-Real-IP when X-Forwarded-For has an invalid first value", () => {
      const proxyLimiter = new RateLimiter(LIMIT, WINDOW_MS, MAX_ENTRIES, true);
      // eslint-disable-next-line sonarjs/no-hardcoded-ip
      const realIp = "8.8.8.8";
      const proxyReq = createMockRequest({
        headers: {
          [HTTP_HEADERS.X_FORWARDED_FOR]: "not-an-ip, 10.0.0.1",
          [HTTP_HEADERS.X_REAL_IP]: realIp,
        },
      });
      proxyLimiter.middleware()(proxyReq, createMockResponse(), createMockNextFunction());
      expect(proxyLimiter.hasIp(realIp)).toBe(true);
      proxyLimiter.destroy();
    });

    it("should set all required headers on 429 response", () => {
      // Deplete limit
      for (let i = 0; i < LIMIT; i++) {
        // eslint-disable-next-line sonarjs/no-hardcoded-ip
        limiter.middleware()(createMockRequest({ ip: "1.2.3.4" }), createMockResponse(), createMockNextFunction());
      }

      const res429 = createMockResponse();
      // eslint-disable-next-line sonarjs/no-hardcoded-ip
      limiter.middleware()(createMockRequest({ ip: "1.2.3.4" }), res429, createMockNextFunction());

      expect(res429.status).toHaveBeenCalledWith(HTTP_STATUS.TOO_MANY_REQUESTS);
      expect(res429.setHeader).toHaveBeenCalledWith(HTTP_HEADERS.RETRY_AFTER, expect.any(Number));
      expect(res429.setHeader).toHaveBeenCalledWith(HTTP_HEADERS.X_RATELIMIT_LIMIT, LIMIT);
      expect(res429.setHeader).toHaveBeenCalledWith(HTTP_HEADERS.X_RATELIMIT_REMAINING, 0);
      expect(res429.setHeader).toHaveBeenCalledWith(HTTP_HEADERS.X_RATELIMIT_RESET, expect.any(Number));
    });

    it("should return MASK_HIDDEN when maskIp is called with null/undefined", () => {
      expect(limiter.maskIp(null)).toBe(LOG_MESSAGES.MASK_HIDDEN);
      expect(limiter.maskIp(undefined)).toBe(LOG_MESSAGES.MASK_HIDDEN);
    });

    it("should block immediately when limit is set to 0", () => {
      limiter.limit = 0;
      const resLocal = createMockResponse();
      const nextLocal = createMockNextFunction();

      // eslint-disable-next-line sonarjs/no-hardcoded-ip
      limiter.middleware()(createMockRequest({ ip: "1.2.3.4" }), resLocal, nextLocal);

      expect(resLocal.status).toHaveBeenCalledWith(HTTP_STATUS.TOO_MANY_REQUESTS);
