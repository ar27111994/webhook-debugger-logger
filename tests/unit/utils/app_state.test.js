...
const { loggerMock, webhookManagerMock, loggerMiddlewareMock } =
...
describe("AppState", () => {
  ...
  const UPDATED_TEST_URL_COUNT = 2;
  const UPDATED_TEST_RETENTION = 72;
  const UPDATED_TEST_RATE_LIMIT = 120;
  const UPDATED_TEST_RETRIES = 5;
  const UPDATED_TEST_TIMEOUT = 60000;
  const UPDATED_TEST_MEMORY = 512;
  ...
  beforeEach(() => {
    ...
    jest
      ...
      .mockReturnValue(new Array(mockConfig.urlCount).fill({}));
  ...
  describe("constructor", () => {
    ...
    it("should use default values for missing config properties", () => {
      ...
      expect(emptyState.replayMaxRetries).toBe(
  ...
  describe("applyConfigUpdate()", () => {
    it("should propagate updates to loggerMiddleware", async () => {
      /** @type {WebhookConfig} */
      const newNormalizedInput = assertType({
        authKey: "new-key",
      });
      const validated = { ...mockConfig, authKey: "new-key" };
      await appState.applyConfigUpdate(newNormalizedInput, validated);
      expect(loggerMiddlewareMock.updateOptions).toHaveBeenCalledWith(
        newNormalizedInput,
      );
    });

    it("should skip loggerMiddleware updates when only non-middleware runtime settings change", async () => {
      /** @type {WebhookConfig} */
      const newNormalizedInput = assertType({
        ...mockConfig,
        replayMaxRetries: UPDATED_TEST_RETRIES,
        replayTimeoutMs: UPDATED_TEST_TIMEOUT,
        retentionHours: UPDATED_TEST_RETENTION,
        urlCount: UPDATED_TEST_URL_COUNT,
        rateLimitPerMinute: UPDATED_TEST_RATE_LIMIT,
        useFixedMemory: true,
        fixedMemoryMbytes: UPDATED_TEST_MEMORY,
      });
      jest.mocked(webhookManagerMock.getAllActive).mockReturnValue([]);

      await appState.applyConfigUpdate(newNormalizedInput, {
        ...mockConfig,
        replayMaxRetries: UPDATED_TEST_RETRIES,
        replayTimeoutMs: UPDATED_TEST_TIMEOUT,
        retentionHours: UPDATED_TEST_RETENTION,
      ...
      expect(loggerMiddlewareMock.updateOptions).not.toHaveBeenCalled();
      expect(appState.replayMaxRetries).toBe(UPDATED_TEST_RETRIES);
      expect(appState.replayTimeoutMs).toBe(UPDATED_TEST_TIMEOUT);
      expect(appState.retentionHours).toBe(UPDATED_TEST_RETENTION);
      expect(appState.rateLimiter.limit).toBe(UPDATED_TEST_RATE_LIMIT);
      expect(appState.useFixedMemory).toBe(true);
      expect(appState.fixedMemoryMbytes).toBe(UPDATED_TEST_MEMORY);
      expect(webhookManagerMock.generateWebhooks).toHaveBeenCalledWith(
        UPDATED_TEST_URL_COUNT,
        DEFAULT_TEST_RETENTION,
      );
      expect(webhookManagerMock.updateRetention).toHaveBeenCalledWith(
        UPDATED_TEST_RETENTION,
      );
    });

    it("should skip loggerMiddleware updates when the effective middleware config is unchanged after normalization", async () => {
      /** @type {WebhookConfig} */
      const newNormalizedInput = assertType({
        ...mockConfig,
        authKey: "  old-key  ",
    ...
    it("should update maxPayloadSize and recreate bodyParser", async () => {
      ...
      expect(bodyParser.raw).toHaveBeenCalledWith(
        expect.objectContaining({ limit: NEW_LIMIT }),
      );
