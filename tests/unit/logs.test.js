...
import { HTTP_HEADERS, HTTP_METHODS, HTTP_STATUS, MIME_TYPES } from '../../src/consts/http.js';
...
await setupCommonMocks({ apify: true, repositories: true, webhookManager: true });
...
const { createLogsHandler, createLogDetailHandler, createLogPayloadHandler } = await import('../../src/routes/logs.js');
...
describe('Logs Routes', () => {
    ...
    describe('createLogsHandler', () => {
        it('should return paginated offset logs correctly and handle defaults', async () => {
            const mockStoredLogs = {
                items: [{ id: '1', webhookId: 'wh1' }, { id: '2', webhookId: 'wh1' }],
                total: 5
            };
            mockLogRepo.findLogs.mockResolvedValueOnce(assertType(mockStoredLogs));

            const handler = createLogsHandler(mockWebhookManager, { logRepo: mockLogRepo });
            await handler(mockReq, mockRes, mockNext);

            expect(mockLogRepo.findLogs).toHaveBeenCalledWith(expect.objectContaining({
            ...
            expect(mockRes.json).toHaveBeenCalledWith(expect.objectContaining({
                ...
                items: expect.arrayContaining([
                    expect.objectContaining({ id: '1', detailUrl: 'https://example.com/admin/api/logs/1' }),
                    expect.objectContaining({ id: '2', detailUrl: 'https://example.com/admin/api/logs/2' })
                ]),
        ...
        it('should return paginated cursor logs efficiently', async () => {
            ...
            mockLogRepo.findLogsCursor.mockResolvedValueOnce(assertType(mockStoredLogs));

            const handler = createLogsHandler(mockWebhookManager, { logRepo: mockLogRepo });
            await handler(mockReq, mockRes, mockNext);

            expect(mockLogRepo.findLogsCursor).toHaveBeenCalledWith(expect.objectContaining({
            ...
            expect(mockRes.json).toHaveBeenCalledWith(expect.objectContaining({
                count: mockStoredLogs.items.length,
                items: expect.arrayContaining([
                    expect.objectContaining({ detailUrl: 'https://example.com/admin/api/logs/cursor1' })
                ]),
                nextCursor: mockStoredLogs.nextCursor,
                nextPageUrl: expect.stringContaining(`cursor=${mockStoredLogs.nextCursor}`)
            }));
        });
        ...
        it('should translate complex parsing parameters (ranges, sort fields) cleanly', async () => {
            ...
            expect(mockLogRepo.findLogs).toHaveBeenCalledWith(expect.objectContaining({
                ...
                timestamp: expect.arrayContaining([
                    { operator: 'gte', value: new Date(String(mockReq.query.startTime)).toISOString() },
                    { operator: 'lte', value: new Date(String(mockReq.query.endTime)).toISOString() }
                ])
            }));
        });
        ...
        it('should fallback to first valid sort field if timestamp is not in valid fields', async () => {
            ...
            Array.prototype.includes = function (searchElement, fromIndex) {
                ...
                if (this && this.length === EXPECTED_FIELDS_LEN && this[0] === 'id' && searchElement === 'timestamp') {
                    return false;
                }
        ...
        it('should correctly parse valid timestamp range queries dynamically', async () => {
            ...
            expect(mockLogRepo.findLogs).toHaveBeenCalledWith(expect.objectContaining({
                timestamp: expect.arrayContaining([
                    expect.objectContaining({ operator: 'eq', value: mockReq.query.timestamp })
                ])
            }));
        });
        ...
        it('should map all query level filters dynamically parsing defaults securely', async () => {
            ...
            jest.mocked(mockLogRepo.findLogs).mockResolvedValueOnce({ items: [], total: 0 });
    ...
    describe('createLogDetailHandler', () => {
        it('should retrieve a log by id missing fields fetching everything internally', async () => {
            ...
            jest.mocked(mockLogRepo.getLogById).mockResolvedValueOnce(assertType(mockLogData));
        ...
        it('should slice requested fields properly, but implicitly include webhookId, stripping it later if unrequested', async () => {
            ...
            jest.mocked(mockLogRepo.getLogById).mockResolvedValueOnce(assertType({ id: 'log-1', method: HTTP_METHODS.POST, webhookId: MOCK_WH_ID }));

            const handler = createLogDetailHandler(mockWebhookManager);
            await handler(mockReq, mockRes, mockNext);

            // Verify webhookId was requested silently for validation
            expect(mockLogRepo.getLogById).toHaveBeenCalledWith('log-1', ['id', 'method', 'webhookId']);

            // Verify it was stripped when delivered
            expect(mockRes.json).toHaveBeenCalledWith({ id: 'log-1', method: HTTP_METHODS.POST });
        });
        it('should return 404 if log is not found', async () => {
            ...
            expect(mockRes.json).toHaveBeenCalledWith({ error: ERROR_MESSAGES.LOG_NOT_FOUND });
        });

        it('should return 404 if the corresponding webhook configuration is no longer active', async () => {
            jest.mocked(mockLogRepo.getLogById).mockResolvedValueOnce(assertType({ id: 'log-1', webhookId: 'deleted-wh' }));
        ...
        it('should return 500 if database fetching throws', async () => {
            jest.mocked(mockLogRepo.getLogById).mockRejectedValueOnce(new Error('Boom'));
            const handler = createLogDetailHandler(mockWebhookManager);
            await handler(mockReq, mockRes, mockNext);
            expect(mockRes.status).toHaveBeenCalledWith(HTTP_STATUS.INTERNAL_SERVER_ERROR);
        });
    ...
    describe('createLogPayloadHandler', () => {
        ...
        it('should return 404 if the log webhook is invalid', async () => {
            ...
            jest.mocked(mockLogRepo.getLogById).mockResolvedValueOnce(assertType({ id: MOCK_LOG_ID, webhookId: 'deleted-wh' }));
            jest.mocked(mockWebhookManager.isValid).mockReturnValueOnce(false);
        ...
        it('should return the body directly if not offloaded', async () => {
            ...
            jest.mocked(mockLogRepo.getLogById).mockResolvedValueOnce(assertType({
                id: MOCK_LOG_ID,
            ...
            expect(mockRes.setHeader).toHaveBeenCalledWith(HTTP_HEADERS.CONTENT_TYPE, MIME_TYPES.JSON);
            expect(mockRes.json).toHaveBeenCalledWith(directBody);
        });

        it('should fetch the payload from Apify KVS if offloaded', async () => {
            mockReq.params = { logId: MOCK_LOG_ID };
            const offloadedPayloadInfo = { data: STORAGE_CONSTS.OFFLOAD_MARKER_SYNC, key: MOCK_KEY };
            jest.mocked(mockLogRepo.getLogById).mockResolvedValueOnce(assertType({
                id: MOCK_LOG_ID,
                webhookId: MOCK_WH_ID,
            ...
            jest.mocked(mockApifyActor.openKeyValueStore).mockResolvedValueOnce(mockStore);

            await handler(mockReq, mockRes, mockNext);

            expect(mockApifyActor.openKeyValueStore).toHaveBeenCalled();
            expect(mockStore.getValue).toHaveBeenCalledWith(MOCK_KEY);
            expect(mockRes.setHeader).toHaveBeenCalledWith(HTTP_HEADERS.CONTENT_TYPE, MIME_TYPES.TEXT);
        ...
        it('should send 404 if KVS payload fetch fails resolving (null value)', async () => {
            mockReq.params = { logId: MOCK_LOG_ID };
            const offloadedPayloadInfo = { data: STORAGE_CONSTS.OFFLOAD_MARKER_STREAM, key: MOCK_KEY };
            jest.mocked(mockLogRepo.getLogById).mockResolvedValueOnce(assertType({
                id: MOCK_LOG_ID,
                webhookId: MOCK_WH_ID,
                body: offloadedPayloadInfo
        ...
        it('should return 500 cleanly on handler operation exceptions', async () => {
            jest.mocked(mockLogRepo.getLogById).mockRejectedValueOnce(new Error('KVS Access Denied'));
