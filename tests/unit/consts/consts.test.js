/**
 * @file tests/unit/consts/consts.test.js
...
describe("Consts Logic", () => {
  ...
  describe("ERROR_MESSAGES", () => {
    ...
    it("should format REPLAY_TIMEOUT correctly", () => {
      // 30000ms / 1000 = 30s
      expect(
        ERROR_MESSAGES.REPLAY_TIMEOUT(RETRY_ATTEMPTS, RETRY_TIMEOUT_MS),
      ).toBe(
    ...
    it("should format FORWARD_FAILURE_DETAILS correctly", () => {
      ...
      expect(
        ...
        `Forwarding to http://example.com failed after ${ATTEMPT_count} attempts. Last error: Timeout`,
      );
      ...
      expect(
        ...
        "Forwarding to http://example.com failed (Non-transient error/Permanent Failure) after 1 attempts. Last error: 400 Bad Request",
      );
    });

    it("should format WEBHOOK_RATE_LIMIT_EXCEEDED correctly", () => {
      expect(
        ERROR_MESSAGES.WEBHOOK_RATE_LIMIT_EXCEEDED(MIN_WEBHOOK_LIMIT),
      ).toBe(
    ...
    it("should format ALERT_URL_BLOCKED_BY_SSRF_POLICY correctly", () => {
      expect(
        ERROR_MESSAGES.ALERT_URL_BLOCKED_BY_SSRF_POLICY("Private IP"),
      ).toBe("Alert URL blocked by SSRF policy: Private IP");
    });
    ...
    it("should format CONDITION_NOT_MET correctly", () => {
      expect(ERROR_MESSAGES.CONDITION_NOT_MET(MOCK_TIMEOUT)).toBe(
