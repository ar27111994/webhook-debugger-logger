...
describe('Config Utils', () => {
    ...
    describe('getSafeResponseDelay', () => {
        ...
        it('should clamp value to MAX_SAFE_RESPONSE_DELAY_MS and log warning if exceeded', () => {
            const excessiveDelay = APP_CONSTS.MAX_SAFE_RESPONSE_DELAY_MS + APP_CONSTS.MS_PER_SECOND;
    ...
    describe('normalizeInput', () => {
        it('should return fallback for null/undefined input', () => {
            ...
            expect(configUtils.normalizeInput(null, assertType(fallback))).toEqual(fallback);
            expect(configUtils.normalizeInput(undefined, assertType(fallback))).toEqual(fallback);
        });
        ...
        it('should return fallback if JSON parsing fails', () => {
            ...
            expect(configUtils.normalizeInput(invalidJson, fallback)).toEqual(fallback);
        });
    });
    ...
    describe('coerceRuntimeOptions', () => {
        it('should use default values for missing or invalid inputs', () => {
            ...
            expect(options.rateLimitPerMinute).toBe(APP_CONSTS.DEFAULT_RATE_LIMIT_PER_MINUTE);
        ...
        it('should clamp excessive values and log warnings', () => {
            ...
            const excessiveInput = {
                ...
                retentionHours: APP_CONSTS.MAX_SAFE_RETENTION_HOURS + RETENTION_HOURS_EXCESSIVE,
                maxPayloadSize: APP_CONSTS.MAX_ALLOWED_PAYLOAD_SIZE + MAX_PAYLOAD_SIZE_EXCESSIVE
            };
        ...
        it('should handle undefined clamp limits gracefully', async () => {
            ...
            const { coerceRuntimeOptions } = await import('../../src/utils/config.js');
        ...
        it('should use default rate limit for invalid (low) values', () => {
            ...
            expect(result.rateLimitPerMinute).toBe(APP_CONSTS.DEFAULT_RATE_LIMIT_PER_MINUTE);
        });

        it('should use default rate limit when input is invalid or negative', () => {
            const result = configUtils.coerceRuntimeOptions({ rateLimitPerMinute: -5 });
            expect(result.rateLimitPerMinute).toBe(APP_CONSTS.DEFAULT_RATE_LIMIT_PER_MINUTE);

            const resultInvalid = configUtils.coerceRuntimeOptions(assertType({ rateLimitPerMinute: 'invalid' }));
            expect(resultInvalid.rateLimitPerMinute).toBe(APP_CONSTS.DEFAULT_RATE_LIMIT_PER_MINUTE);
        });

        it('should use provided rate limit if valid', () => {
            const validLimit = 100;
            const result = configUtils.coerceRuntimeOptions({ rateLimitPerMinute: validLimit });
            expect(result.rateLimitPerMinute).toBe(validLimit);
        });

        it('should trim authKey', () => {
            const correctAuthKey = 'secret';
            const options = configUtils.coerceRuntimeOptions({ authKey: `  ${correctAuthKey}  ` });
        ...
        it('should clamp replay options', () => {
            ...
            expect(options.replayTimeoutMs).toBe(APP_CONSTS.MAX_SAFE_REPLAY_TIMEOUT_MS);
            expect(options.maxForwardRetries).toBe(APP_CONSTS.MAX_SAFE_FORWARD_RETRIES);
        ...
        it('should clamp fixed memory options', () => {
            ...
            const options = configUtils.coerceRuntimeOptions(input);
            expect(options.fixedMemoryMbytes).toBe(APP_CONSTS.MAX_SAFE_FIXED_MEMORY_MBYTES);
            expect(loggerMock.warn).toHaveBeenCalledWith(
    ...
    describe('parseWebhookOptions', () => {
        it('should apply defaults for complex fields', () => {
            ...
            expect(result.defaultResponseCode).toBe(HTTP_CONSTS.DEFAULT_RESPONSE_CODE);
            expect(result.maskSensitiveData).toBe(APP_CONSTS.DEFAULT_MASK_SENSITIVE_DATA);
        });
